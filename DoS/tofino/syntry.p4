...
parser start {
	...
	//set_metadata(meta.in_port,ig_intr_md.ingress_port);
...
{//from packet come in through port1
	...
	//bit_and(meta.tcp_synack,meta.tcp_syn,meta.tcp_ack);
...
{//for packet come in through port2, get the reverse-hash
	...
	//modify_field_with_hash_based_offset(meta.dstip_pktcount_map_index,0,dstip_map_hash,8);
...
action read_state_SYN_action(){
	...
	//read_tcp_session_is_SYN.execute_stateful_alu_from_hash(tcp_session_map_hash);
}
...
action read_state_ACK_action(){
	...
	//read_tcp_session_is_ACK.execute_stateful_alu_from_hash(tcp_session_map_hash);
}
table read_state_h2 {
	reads {
		tcp.syn:exact;
...
action init_session()
{
	...
	//register_write(tcp_session_is_SYN, meta.tcp_session_map_index,
					//1);
	//set_tcp_session_is_SYN.execute_stateful_alu(meta.tcp_session_map_index);
